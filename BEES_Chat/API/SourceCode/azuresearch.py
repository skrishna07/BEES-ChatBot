...
class AzureSearch(VectorStore):
    ...
    def hybrid_search_with_score(
        ...
        **kwargs: Any,
        ...
        results = _results_to_documents(results)
        category = ''
        source = ''
        ...
        k = 5
        filters = None
        if category != 'BusPageMenu':
            if source != "https://beep.biologicale.com/NavigateUrl?path=LinkPage-SHAMEERPET_INTER_OFFICE_SHUTTLE_SERVICE_TIMMINGS":
                filters = f"metadata/source eq '{source}'"
                k = 3
        results = self._simple_search(embedding, query, k, filters=filters, **kwargs)
        results = _results_to_documents(results)
        return results
